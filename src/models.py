...
class AttentionHead(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v):
        ...
        Args:
            q (Tensor): Query tensor of shape (batch_size, seq_len, d_q). Hazme esto
    ...
    def forward(self, x):
        ...
        d_q = self.wq.out_features
        d_k = self.wk.out_features
        d_v = self.wv.out_features

        q = self.wq(x).view(B, S, d_q)
        k = self.wk(x).view(B, S, d_k)
        v = self.wv(x).view(B, S, d_v)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, hidden_state):
        ...
        for head in self.heads:
            x.append(head(hidden_state))

        x = torch.cat(x, dim=-1)
        x_out = self.output_linear(x)
        return x_out
